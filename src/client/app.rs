...
pub struct App {
    ...
    pub footer_chat_area: Rect,
    pub footer_alerts_area: Rect,
    pub footer_logs_area: Rect,
    pub footer_terminal_area: Rect,
    pub footer_docs_area: Rect,
    pub kb_docs: Vec<KbDoc>,
    pub kb_selected: usize,
    pub kb_list_area: Rect,
}

impl App {
    fn default_chat_threads() -> Vec<ChatThread> {
        crate::client::agentchattr::load_chat_threads()
    }

    fn default_kb_docs() -> Vec<KbDoc> {
        crate::client::docs_index::load_kb_docs()
    }
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            chat_threads: Self::default_chat_threads(),
            chat_selected: 0,
            footer_chat_area: Rect::default(),
            footer_alerts_area: Rect::default(),
            footer_logs_area: Rect::default(),
            footer_terminal_area: Rect::default(),
            footer_docs_area: Rect::default(),
            kb_docs: Self::default_kb_docs(),
            kb_selected: 0,
            kb_list_area: Rect::default(),
        }
    }
    ...
    pub fn select_next_chat(&mut self) {
        if self.chat_threads.is_empty() {
            self.chat_selected = 0;
            return;
        }
        if self.chat_selected + 1 < self.chat_threads.len() {
    ...
    pub fn select_next_doc(&mut self) {
        if self.kb_docs.is_empty() {
            self.kb_selected = 0;
            return;
        }
        if self.kb_selected + 1 < self.kb_docs.len() {
            self.kb_selected += 1;
        }
    }
    ...
    pub fn select_prev_doc(&mut self) {
        ...
        self.kb_selected = self.kb_selected.saturating_sub(1);
    }
