...
pub(super) fn draw_terminal(f: &mut Frame, app: &mut App, area: Rect, t: &ThemeColors) {
    // Clear the whole terminal panel first to avoid stale glyph artifacts
    // when switching tabs/sizes and when content shrinks between frames.
    ...
    // Reserve one column for a potential scrollbar to avoid content overdraw.
    let content_w = iw.saturating_sub(1).max(1);
    let screen = tab.provider.get_screen(ih, content_w);
    ...
    // Keep the reserved scrollbar column clean by default.
    if iw > content_w {
        for y in 0..ih {
            if let Some(cell) = buf.cell_mut((ix + content_w, iy + y)) {
                cell.set_char(' ');
    ...
    // Draw a right-side scrollbar when scrollback is available.
    if iw > content_w && ih > 0 && screen.scroll_max > 0 {
        let bar_x = ix + content_w;
        let track_style = Style::default().bg(term_bg).fg(t.fg_dim());
        let thumb_style = Style::default().bg(term_bg).fg(t.accent());
        let max = screen.scroll_max as u32;
        let offset = (screen.scroll_offset as u32).min(max);
        let track_h = ih as u32;
        let thumb_h = ((track_h * track_h) / (track_h + max)).max(1).min(track_h);
        let travel = track_h.saturating_sub(thumb_h);
        // offset=0 => thumb at bottom, offset=max => thumb at top.
        let thumb_top = if max == 0 {
            0
        } else {
            ((max.saturating_sub(offset)) * travel) / max
        };

        for y in 0..ih {
            if let Some(cell) = buf.cell_mut((bar_x, iy + y)) {
                cell.set_char('│');
                cell.set_style(track_style);
            }
        }
        for y in thumb_top..(thumb_top + thumb_h) {
            if let Some(cell) = buf.cell_mut((bar_x, iy + y as u16)) {
                cell.set_char('█');
    ...
    if !app.copy_mode && ih > 0 && content_w > 0 && screen.cursor_shape != CursorShape::Hidden {
        let cursor_row = cr.min(screen_rows.saturating_sub(1));
        let cursor_col = cc.min(content_w.saturating_sub(1));
        ...
        let cursor_x = ix + visual_cc.min(content_w.saturating_sub(1));
