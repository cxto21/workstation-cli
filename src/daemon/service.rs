            ...
            ClientMsg::Subscribe { tab_id, rows, cols } => {
                ...
                loop {
                    ...
                    let bin = if let Some(ref prev) = last_sent_screen {
                        ...
                        for i in 0..max_lines {
                            ...
                            if new_line != old_line {
                                if let Some(line) = new_line {
                                    ...
                                    // Line was removed in the new screen. Send an explicit
                                    // empty line so clients can clear stale content.
                                    let blank = crate::terminal_provider::ScreenLine {
                                        cells: vec![crate::terminal_provider::ScreenCell {
                                            ch: ' ',
                                            display_width: 1,
                                            fg: None,
                                            bg: None,
                        ...
                        // Hotfix: force full screen frames in push mode.
                        // Incremental ScreenDiff can leave stale artifacts in some
                        // terminals during rapid tab switches/resizes.
                        let response = ServerMsg::Screen {
                            tab_id: tab_id.clone(),
                            content: content.clone(),
                        };
