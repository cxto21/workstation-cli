...
fn main() -> Result<()> {
    ...
    if want_version {
        println!("workstation-cli {}", env!("CARGO_PKG_VERSION"));
        return Ok(());
    }
...
fn confirm_daemon_restart(client_version: &str, daemon_version: &str) -> bool {
    ...
    eprintln!("- close other running workstation-cli clients");
...
fn print_help() {
    println!(
         ...
         Usage:\n\
              workstation-cli                    Start client UI (auto-start daemon if needed)\n\
              workstation-cli --daemon           Run daemon in background mode\n\
              workstation-cli --daemon --foreground\n\
