        ...
        ServerMsg::ScreenDiff {
            ...
            for (idx, line) in changed_lines {
                ...
                if i >= screen.lines.len() {
                    screen.lines.resize_with(i + 1, || ScreenLine { cells: vec![] });
                }
                screen.lines[i] = line;
            }
...
#[test]
fn removed_line_is_explicitly_cleared() {
    let prev = make_screen(&["line0", "line1", "line2"]);
    let next = make_screen(&["line0", "line1"]);

    let diff = compute_diff(&prev, &next).unwrap();
    match diff {
        ServerMsg::ScreenDiff { changed_lines, .. } => {
            // third line must be present as a clear operation
            let l2 = changed_lines
                .iter()
                .find(|(idx, _)| *idx == 2)
                .expect("missing clear for removed line");
            assert!(!l2.1.cells.is_empty(), "clear line should not be empty");
        }
        _ => panic!("Expected ScreenDiff"),
    }
}
...
#[test]
fn apply_diff_extends_cache_for_new_lines() {
    let mut cached = make_screen(&["a"]);
    let msg = ServerMsg::ScreenDiff {
        changed_lines: vec![(2, make_line("c"))],
        cursor: (0, 0),
        cursor_shape: CursorShape::Block,
        title: None,
        bell: false,
    ...
    apply_diff(&mut cached, msg);
    assert!(cached.lines.len() >= 3);
    assert_eq!(cached.lines[2], make_line("c"));
}
